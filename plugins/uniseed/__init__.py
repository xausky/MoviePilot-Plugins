...
class TorInfo:
    ...
    @staticmethod
    def local(torrent_path: str, info_hash: str, pieces_hash: str, size: int = None):
        return TorInfo(
            torrent_path=torrent_path, info_hash=info_hash, pieces_hash=pieces_hash, size=size
        )

    @staticmethod
    def remote(site_name: str, pieces_hash: str, torrent_id: str, size: int = None):
        return TorInfo(
            site_name=site_name, pieces_hash=pieces_hash, torrent_id=torrent_id, size=size
        )
...
class UniSeed(_PluginBase):
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "uni_seed_"
    # 加载顺序
    plugin_order = 17
    # 可使用的用户级别
    auth_level = 2
        ...
        if self.get_state():
            ...
            if self._cron:
                ...
                for trigger in triggers:
                    ret_jobs.append({
                        "id": f"UniSeed|{trigger.hour}:{trigger.minute}",
                        "name": "UNI辅种助手",
                        "trigger": "cron",
                        "func": self.auto_seed,
                        "kwargs": {
            ...
            logger.warn(f"UNI辅种助手插件参数不全，定时任务未正常启动")
        return []
    ...
    def __seed_torrents(self, hash_strs: list, downloader: str):
        ...
        chunk_size = 100
        for site_config in self._site_cs_infos:
            logger.info(f"开始辅种站点：{site_config.name}")
            # 检查站点是否已经停用
            ...
            if site_config.passkey != "uni":
                for i in range(0, len(pieces_hashes), chunk_size):
                    ...
                    # 处理分组
                    chunk_tors, err_msg = self.cross_helper.get_target_torrent_by_api(site_config, chunk)
                ...
                logger.info(f"开始 UNI 辅种站点：{site_config.name}")
                chunk_tors, err_msg = self.cross_helper.get_target_torrent_by_uni(site_config, hash_strs, self)
                if not chunk_tors and err_msg:
                    logger.info(
                        f"查询站点{site_config.name}可辅种的信息出错 {err_msg}"
                    )
                else:
                    logger.info(
                        f"站点{site_config.name}本批次的可辅种={len(chunk_tors)}"
                    )
                    remote_tors = remote_tors + chunk_tors
            ...
            for torrent in torrents:
                ...
                if torrent.hash is None:
                    ...
                    if not torrent_content:
                        ...
                        return None
                    _, content, _, _, error_msg = seed.torrent.download_torrent(                        
                    url=torrent_content,
                    cookie=site.cookie,
                    ua=site.ua or settings.USER_AGENT,
                    proxy=True if site.proxy else False)
                    if not content or (isinstance(content, bytes) and "你没有该权限".encode(encoding="utf-8") in content):
                        logger.error(f"获取下载链接失败：{torrent.title}, {error_msg}")
                        continue
                    tmp_tor_info, err_msg = TorInfo.from_data(content)
                    if not tmp_tor_info:
                        logger.error(f"获取种子信息失败：{torrent.title}, {err_msg}")
                        continue
                    torrent.hash = tmp_tor_info.pieces_hash
